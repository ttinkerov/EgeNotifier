...
class RustestClient:
    ...
    async def fetch_captcha(self) -> CaptchaPayload | None:
        ...
        except (*_NETWORK_ERRORS, KeyError, TypeError) as exc:
    ...
    async def fetch_scores(self, session_token: str, *, attempts: int = 3) -> list[ExamScore] | None:
            ...
            try:
                async with self._session().get(
                    ...
                    if resp.status != 200:
                        ...
                        continue
                    data = await resp.json()
                    raw_exams = data["Result"]["Exams"]
                    return [ExamScore.model_validate(item) for item in raw_exams]
            except (*_NETWORK_ERRORS, KeyError, ValueError) as exc:
